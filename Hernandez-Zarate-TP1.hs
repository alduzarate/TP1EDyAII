...
search (x:y:xs) (Leaf k v) = "La palabra buscada no está en el dic"
search [x] (Leaf k v) = if (x = k) then v else error "La palabra buscada no está en el dic"
search [x] (Node k v i m d) = if (k = x) 	then v
											else 
												if(x > k) search [x] d else search x i
